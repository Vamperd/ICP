...
% 简洁版 PL-ICP 主程序，结构参考 syk IPC/pl_icp_kd.m
data_dir = fullfile(pwd, 'ICP_data');
output_dir = fullfile(pwd, 'results_matlab', 'plicp');
...
for i = 1:9
    fprintf('myplicp mapping: pair %d -> %d\n', i - 1, i);
    curr_ply = pcread(fullfile(data_dir, sprintf('%d.ply', i)));

    [tform_init, curr_ply, debug_info] = myplicp(curr_ply, laser_map, tform_init, 50, 0.01, 0.6, 0.1);
...
disp('================ My PL-ICP Metrics ================');
disp(metrics_table);
disp('================ My PL-ICP Global Poses ================');
disp(global_poses);

save(fullfile(output_dir, 'result_summary.mat'), 'robot_tf', 'global_poses', 'metrics_table', 'MSE');

plot_map_and_trajectory(laser_map, robot_tf, fullfile(output_dir, 'local_map.png'), 'My PL-ICP 地图与轨迹');
plot_topdown_trajectory(robot_tf, fullfile(output_dir, 'trajectory_topdown.png'), 'My PL-ICP 俯视轨迹图');
plot_mse_overview(MSE, metrics_table, fullfile(output_dir, 'mse_overview.png'), 'My PL-ICP 9 次配准 MSE');
